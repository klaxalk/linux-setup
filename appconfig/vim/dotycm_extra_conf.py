...
def GetCompilationInfoForFile(filename, database, debug=False):
    ...
    if IsHeaderFile(filename):
        if debug:
            print("The requested file \"{}\" is a header file.".format(filename))
...
def Settings(**kwargs):
    ...
    if debug:
        print("Compilation folder: \"{}\"".format(folder))
    database = GetDatabase(folder)
    if database:
        # Bear in mind that compilation_info.compiler_flags_ does NOT return a
        # python list, but a "list-like" StringVec object
        compilation_info = GetCompilationInfoForFile(filename, database, debug=debug)
        if compilation_info:
