...
if [[ "$MONITOR" != "GPU" ]] && [[ "$MONITOR" != "LAB" ]] && [[ "$MONITOR" != "HOME" ]] && [[ "$MONITOR" != "PRESENTATION" ]] && [[ "$MONITOR" != "STANDALONE" ]]; then
...
ln -sf $GIT_PATH/linux-setup/miscellaneous/arandr_scripts/daniel/$MONITOR_LOWERCASE.sh ~/.monitor.sh
